'use strict';
angular.module('oneClickApp').controller('TransitController', [
  '$scope',
  '$routeParams',
  '$location',
  'flash',
  ...
  function ($scope, $routeParams, $location, flash, planService, $http, ipCookie, $attrs) {
    if ($routeParams.departid && $attrs.segmentid > -1) {
      ...
      $scope.tripid = $scope.segmentid == 0 ? $routeParams.departid : $routeParams.returnid;
      //$scope.$parent.transitInfos[$attrs.segmentid][0].id;
      $scope.embedded = true;
    } else {
    ...
    $scope.generateLeg = function (leg, index, selected, legLines) {
      ...
      var routeColors = {
        ...
        'secondary': {
          'walk': 'hsla(198, 0%, 50%, .7)',
          'bus': 'hsla(198, 0%, 50%, .7)',
          'bike': 'hsla(198, 0%, 50%, .7)',
          'car': 'hsla(33, 0%, 50%, .7)'
        }
      };
      legLines.push({
      ...
      if (leg.mode != 'WALK') {
        ...
        boxText.innerHTML = '<img src="' + imageUrl + '" style="width: 16px; height: 16px;"/>' + '<span style="margin: 0px 1px 0px 2px; position: relative; padding: 0px 4px; color: rgb(255, 255, 255); background-color: ' + routeColor + '; color: ' + routeTextColor + '">' + routeShortName + '</span>' + '<img src="http://maps.gstatic.com/mapfiles/tiph.png" draggable="false" style="-webkit-user-select: none; border: 0px; padding: 0px; margin: 0px; position: absolute; right: -7px; top: 17px; width: 15px; height: 9px;"/>';
